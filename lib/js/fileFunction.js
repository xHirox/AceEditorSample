...
/* drag event */
function EV_Drag(event) {
    event.preventDefault();
}

/* === file open === */
/* confirm filesave before open */
function conf_FileSave() {
    if (editchange) {
...
function getFileInfo(input) {
    var fileinfo = [
        ...
        "last modidied: " + input.lastModifiedDate.toLocaleDateString(),
        "urn: " + input.urn
    ];
...
/* open file set */
function setOpenFileName(name, ext) {
    document.getElementById("open-filename").innerText = name;
    editor.getSession().setMode("ace/mode/" + ext);
    editchange = false;
}

/* fileopen error massege set */
function setErrMsg_FileOpen() {
    var errmsg = [
        "",
        "NOT_FOUND_ERR\nファイルが見つかりません",
        "SECURITY_ERR\nセキュリティエラー",
...
/* text file read from fileopen button */
function fileOpen() {
    conf_FileSave();
...
/* text file read from drag file */
function fileOpen_Drag(event) {
    event.preventDefault();
